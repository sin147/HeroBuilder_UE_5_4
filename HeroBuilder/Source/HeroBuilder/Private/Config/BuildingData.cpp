// Fill out your copyright notice in the Description page of Project Settings.


#include "Config/BuildingData.h"
#if WITH_EDITOR
#include "UObject/UObjectIterator.h"
#endif

FBuildingConfig UBuildingData::GetBuildingInfoByBuildingClass(TSubclassOf<AHB_Building_Base> BuildingClass)
{
	FBuildingConfig RetBuildingConfig;
	if (BuildingInfoMap.Contains(BuildingClass))
	{
        return BuildingInfoMap[BuildingClass];
	}
    return RetBuildingConfig;
}

UStaticMesh* UBuildingData::GetPreviewMeshByBuildingClass(TSubclassOf<AHB_Building_Base> BuildingClass)
{
	UStaticMesh* RetPreviewMesh = nullptr;
	if (BuildingInfoMap.Contains(BuildingClass))
	{
        RetPreviewMesh = BuildingInfoMap[BuildingClass].PreviewMesh;
    }
    return RetPreviewMesh;
}

#if WITH_EDITOR
void UBuildingData::PostInitProperties()
{
	Super::PostInitProperties();
	if (!HasAnyFlags(RF_ClassDefaultObject | RF_NeedLoad | RF_NeedPostLoad))
	{
		RefreshBuildingInfoMap();
	}
}

void UBuildingData::PostLoad()
{
	Super::PostLoad();
	RefreshBuildingInfoMap();
}

void UBuildingData::RefreshBuildingInfoMap()
{
	// 收集所有AHB_Building_Base的子类（仅蓝图实例化用的真实子类）
	TSet<TSubclassOf<AHB_Building_Base>> AllBuildingClasses;
	for (TObjectIterator<UClass> It; It; ++It)
	{
		UClass* Class = *It;
		if (!Class || !Class->IsChildOf(AHB_Building_Base::StaticClass()))
		{
			continue;
		}
		// 跳过基类自身（基类不直接实例化，仅作为子类继承根）
		if (Class == AHB_Building_Base::StaticClass())
		{
			continue;
		}
		// 跳过抽象类、废弃类、被新版本替换的占位类、隐藏类、不可放置类
		if (Class->HasAnyClassFlags(CLASS_Abstract | CLASS_Deprecated | CLASS_NewerVersionExists | CLASS_Hidden | CLASS_HideDropDown))
		{
			continue;
		}
		// 跳过SKEL_/REINST_/TRASHCLASS_/HOTRELOADED_等编译期/热重载占位类
		const FString ClassName = Class->GetName();
		if (ClassName.StartsWith(TEXT("SKEL_")) ||
			ClassName.StartsWith(TEXT("REINST_")) ||
			ClassName.StartsWith(TEXT("TRASHCLASS_")) ||
			ClassName.StartsWith(TEXT("HOTRELOADED_")) ||
			ClassName.StartsWith(TEXT("PLACEHOLDER-CLASS_")))
		{
			continue;
		}
		// 跳过瞬态/正在被GC的类
		if (Class->HasAnyFlags(RF_ClassDefaultObject | RF_NewerVersionExists | RF_BeginDestroyed))
		{
			continue;
		}
		AllBuildingClasses.Add(Class);
	}

	bool bChanged = false;

	// 主动剔除无效/重复/异常Key
	// 1) nullptr Key
	// 2) 不在AllBuildingClasses中的Key（包括抽象类、废弃类、REINST_占位类等）
	// 3) 同一UClass*的重复Key
	TSet<UClass*> SeenClasses;
	for (auto It = BuildingInfoMap.CreateIterator(); It; ++It)
	{
		UClass* KeyClass = It.Key();

		// 无效Key
		if (!KeyClass)
		{
			UE_LOG(LogTemp, Warning, TEXT("BuildingData: 剔除无效Key(nullptr)"));
			It.RemoveCurrent();
			bChanged = true;
			continue;
		}

		// 异常/不再合法的Key
		if (!AllBuildingClasses.Contains(KeyClass))
		{
			UE_LOG(LogTemp, Warning, TEXT("BuildingData: 剔除非法Key %s"), *KeyClass->GetName());
			It.RemoveCurrent();
			bChanged = true;
			continue;
		}

		// 重复Key（保险逻辑）
		bool bAlreadySeen = false;
		SeenClasses.Add(KeyClass, &bAlreadySeen);
		if (bAlreadySeen)
		{
			UE_LOG(LogTemp, Warning, TEXT("BuildingData: 剔除重复Key %s"), *KeyClass->GetName());
			It.RemoveCurrent();
			bChanged = true;
		}
	}

	// 添加新出现的子类（已存在的保留原配置）
	for (const TSubclassOf<AHB_Building_Base>& BuildingClass : AllBuildingClasses)
	{
		if (!BuildingInfoMap.Contains(BuildingClass))
		{
			BuildingInfoMap.Add(BuildingClass, FBuildingConfig());
			bChanged = true;
		}
	}

	if (bChanged)
	{
		MarkPackageDirty();
	}
}
#endif